function [hfig,DE] = parse_and_plot_new3(dim,r_fac,folders,vars,var_mask,norms,iterations,layers,tags,fmt,color_spec,legend_flag)
if(nargin<11)
    legend_flag = false;
end

N = numel(vars);
Nvars = numel(var_mask{1});
if isscalar(folders)
...
for i = 1:N
    ...
    if size(color_spec{i},1)==1
        color_spec{i} = repmat(color_spec{i},N_vars,1);
    end
end
...
base_legend_text_primitive = {'$\\rho$ %s','$u$ %s','$v$ %s','$w$ %s','$p$ %s'};
base_legend_text_conserved = {'$\\rho{\\,\\,\\,}$ %s','$\\rho{u}$ %s','$\\rho{v}$ %s','$\\rho{w}$ %s','$\\rho{e}$ %s'};

if (Nvars==6) % SA
    base_legend_text_primitive = [base_legend_text_primitive,'$\\tilde{\\nu}$ %s'];
    base_legend_text_conserved = [base_legend_text_conserved,'$\\rho\\tilde{\\nu}$ %s'];
elseif (Nvars==7) % k-omega
    base_legend_text_primitive = [base_legend_text_primitive,'$k$ %s','$\\omega$ %s'];
    base_legend_text_conserved = [base_legend_text_conserved,'$\\rho{k}$ %s','$\\rho\\omega$ %s'];
end
...
if legend_flag
    ...
    if (vars(i) == 1)||(vars(i) == 3)||(vars(i) == 4)
        legend_text = cellfun(@(str)sprintf(str,''),base_legend_text_primitive,'UniformOutput',false);
        legend_text = legend_text(tmp_mask);
    elseif (vars(i) == 2)||(vars(i) == 5)||(vars(i) == 6)
        legend_text = cellfun(@(str)sprintf(str,''),base_legend_text_conserved,'UniformOutput',false);
        legend_text = legend_text(tmp_mask);
    end
    if N_plot_var>1
        if N_plot_var<N
...
hold on
if legend_flag
    p = plot_all_vars_legend_flag(gca,tmp_mask,color_spec{i});
    if (sum(tmp_mask)>1)
        if N_plot_var<N
            for j = 1:N-N_plot_var
...
function p = plot_all_vars_legend_flag(ax,mask,colorspec)
hold on
if nargin>2 && size(colorspec,1)>=numel(mask)
    colors = colorspec;
else
...
vars = find(mask);
for i = 1:numel(vars)
    p{i} = plot(ax,nan,nan,'color',colors(vars(i),:));
end
for i = 1:numel(vars)
    p{i}.Marker=markers{vars(i)};
    p{i}.MarkerEdgeColor=colors(vars(i),:);
    p{i}.MarkerSize=3;
    p{i}.LineStyle='none';
end
end
...
hold on
vars = find(var_mask);
if nargin>6 && size(colorspec,1)>=numel(var_mask)
    colors = colorspec;
else
    colors = default_colors;
end
...
for i = 1:numel(vars)
    for j = 1:numel(iterations)
        if isscalar(iterations)
            ...
            alpha = alpha_calc( j/numel(iterations) );    
        end
        [ptmp,Ntmp,DEtmp] = plot_DE(ax,DE,vars(i),norm,iterations(j)+1,layers{1},{linspec,'color',[colors(vars(i),:),alpha],'HandleVisibility','off'});
        scatter(Ntmp,DEtmp,3,markers{vars(i)},'MarkerEdgeColor',colors(vars(i),:),'MarkerEdgeAlpha',alpha,'LineWidth',ptmp.LineWidth,'HandleVisibility','off');
    end
    if (isempty(iterations))
        p{i} = plot_DE(ax,DE,vars(i),norm,[],layers{1},{linspec,'color',colors(vars(i),:)});
    else
        p{i} = plot_DE(ax,DE,vars(i),norm,iterations(end)+1,layers{1},{linspec,'color',colors(vars(i),:)});
    end
    p{i}.Marker=markers{vars(i)};
    p{i}.MarkerEdgeColor=colors(vars(i),:);
    p{i}.MarkerSize=3;
...
for i = 1:numel(vars)
    for j = 1:numel(iterations)
        ...
        [ptmp,Ntmp,OOAtmp] = plot_OOA(ax,DE,vars(i),norm,iterations(j)+1,layers{1},{linspec,'color',[colors(vars(i),:),alpha],'HandleVisibility','off'});
        scatter(Ntmp,OOAtmp,9,markers{vars(i)},'MarkerEdgeColor',colors(vars(i),:),'MarkerEdgeAlpha',alpha,'LineWidth',ptmp.LineWidth,'HandleVisibility','off');
    end
    if (isempty(iterations))
        p{i} = plot_OOA(ax,DE,vars(i),norm,[],layers{1},{linspec,'color',colors(vars(i),:)});
    else
        p{i} = plot_OOA(ax,DE,vars(i),norm,iterations(end)+1,layers{1},{linspec,'color',colors(vars(i),:)});
    end
    p{i}.Marker=markers{vars(i)};
    p{i}.MarkerEdgeColor=colors(vars(i),:);
    p{i}.MarkerSize=3;
...
function m = default_markers()
m = {'o','^','v','>','s','d','p'};
end

function c = default_colors()
c = lines(7);
end
