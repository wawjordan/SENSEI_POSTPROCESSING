...
folders = foldernames1;
ALL_DATA = struct();
ALL_DATA.inputs = inputs;
ALL_DATA.DATA = struct();
for i = 1:numel(folders)
    folder = folders{i};
    ALL_DATA.DATA(i).n_skip = get_nskip_from_folder(folder);
...
function nskip = get_nskip_from_folder(folder)
    file_name = get_nml_file(folder);
    nskip1 = get_nskip_from_file(file_name);
    nskip = nskip1(1);
end

function file_name = get_nml_file(folder)
TMP = dir(fullfile(folder,'**\','sensei.nml'));
% just need one
file_name = fullfile(TMP(1).folder,TMP(1).name);
end


function nskip = get_nskip_from_file(file)
    tmp1 = extractBetween(fileread(file),'GRID_NSKIP',newline);
    tmp2 = cellfun(@str2double,regexp(tmp1,'\d+','match'),'UniformOutput',false);
    nskip = cell2mat(tmp2);
end