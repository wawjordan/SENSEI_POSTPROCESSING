...
legend_labels = {'$C_L$ primal','$C_L$ primal (reconstructed)','$C_L$ corr. (k-exact)','$C_L$ corr. (CWENO)'};
...
post_plot_commands1 = {"set(hfig.Children(4),'Ylim',[1e-7,1e0]);",...
...
folders2 = {ALL_DATA.DATA([17,17,18]).folder};
...
name ='CL_error_anim_ic20_CWENO_CL_and_normserror_only.gif';
filename = fullfile(target_folder,name);
framerate = 2;
for i = 0:20
    ics = {[0],[0],[i],[i]};
    [N,E,OOA] = gather_error_plot_data(ALL_DATA,folders,geom,prim,var,ics);
    hfig1 = plot_OOA(N,E,OOA,linspec,colors,markers,marker_size,x_label1,y_label1,x_label2,y_label2,legend_labels,post_plot_commands1);
    label_iteration(hfig1.Children(4),i);
    label_iteration(hfig1.Children(2),i);
    exportgraphics(hfig1.Children(4),'tmp1.tiff','Resolution',600)
    ...
    exportgraphics(hfig2.Children(4),'tmp2.tiff','Resolution',600)
    I = imtile(["tmp2.tiff","tmp1.tiff"],BackgroundColor="w");
    delete('tmp1.tiff','tmp2.tiff');
