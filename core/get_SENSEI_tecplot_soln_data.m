function DATA = get_SENSEI_tecplot_soln_data(file_name)

ZONES = tec2mat_MOD2_local(file_name);
DATA = struct();
for i = 1:numel(ZONES)
    [DATA_TMP,meta_data] = get_structured_from_sensei_tec2mat_local(ZONES,i);
    DATA(i).ZONE = make_named_struct_local(DATA_TMP);
    DATA(i).meta_data = meta_data;
end
...
for i = 1:N
    ...
    inputs{cnt} = strrep(inputs{cnt},'GREEK_R','RHO');
    inputs{cnt} = strrep(inputs{cnt},'x_RHO','RHO');
    inputs{cnt} = strrep(inputs{cnt},'GREEK','');
    inputs{cnt} = regexprep(inputs{cnt},'(?=\w*)[_]*','_');
    inputs{cnt} = regexprep(inputs{cnt},'(?=\w*)[_]*$','');
    cnt = cnt + 1;
...
function [DATA,metadata] = get_structured_from_sensei_tec2mat_local(ZONES,zone_num)
...
metadata = ZONE;
metadata = rmfield(metadata,{'data_cell','data_nodal'});
...
for n = 1:nvars
    ...
    if ZONE.VARLOC(n) == 0 % nodal
        ...
        % add cell-centered coordinates ( simple average first )
        if any(strcmp(DATA(n+o).name,{'X','Y','Z'}))
            ...
            if (kmax > 2)
                ...
                for k = 1:max(hi(3)-1,1) % case for 2D soln data
                    for j = 1:hi(2)-1
                        for i = 1:hi(1)-1
                            DATA(n+o).data(i,j,k) = mean(DATA(n+o-1).data(i:i+1,j:j+1,:),"all");
                        end
                    end
                end
            end
...
% now calculate more accurate cell averages and add volumes
% assumes we have XC
if (any(strcmp({DATA.name},'XC')))
    o = o + 1;
    DATA(nvars+o).name = 'VOL';
    sz = size(DATA(strcmp({DATA.name},'XC')).data,[1,2,3]);
    DATA(nvars+o).data = zeros(sz(1),sz(2),sz(3));
    
    if jmax > 1
        if kmax > 1
            error('TODO: implement 3D')
        else
            [tmpXC,tmpYC,tmpV] = quad_grid_centroids( ...
                                     DATA(strcmp({DATA.name},'X')).data, ...
                                     DATA(strcmp({DATA.name},'Y')).data);
            DATA(strcmp({DATA.name},'XC')).data = tmpXC;
            DATA(strcmp({DATA.name},'YC')).data = tmpYC;
            DATA(strcmp({DATA.name},'VOL')).data = tmpV;
        end
    else % 1D
        error('TODO: implement 1D')
    end
end
...
%                     varloc = strsplit(varloc{1}{1},{','});
            ...
            if isfield(Prop,'I')
                ...
                if isfield(Prop,'VARSHARELIST')
                    tmp_var = extractBetween(Prop.VARSHARELIST,'[',']');
                    tmp_var = cellfun(@(a)regexp(a,'\d*','match'),tmp_var,'UniformOutput',false);
                    var_share_list = cellfun(@(a)cellfun(@str2num,a),tmp_var,'UniformOutput',false);
                    tmp_var = regexp(Prop.VARSHARELIST,'(?<=[=])\d*','match');
                    zone_share_list = cellfun(@str2num,tmp_var,'UniformOutput',false);
                    Prop.VARSHARELIST = [var_share_list,zone_share_list];
                end
                ...
                if Prop.IVARLOC
                    ...
                    if isfield(Prop,'VARSHARELIST')
                        for ii = 1:numel(var_share_list)
                            for jj = 1:numel(var_share_list{ii})
                                if VARLOC(var_share_list{ii}(jj))==0
                                    ndata_nodal = ndata_nodal - ndata;
                                elseif VARLOC(var_share_list{ii}(jj))==1
                                    ndata_cell = ndata_cell - prod(Ncells);
                                end
                            end
                        end
                        VARLOC(unique([var_share_list{:}]))=[];
                        Prop.VARLOC = VARLOC;
                        ZONEVARlist(unique([var_share_list{:}]))=[];
                        % VARLOC(unique([var_share_list{:}]))=[];
                    end
